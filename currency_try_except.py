...
def inquiry(s):
    ...
    curs.execute(f'''SELECT Rate FROM ccy 
                        WHERE Abbreviation = '{s}' and (strftime('%s') - last_updated <= 20)''')  # запрос на вывод значения с таблицы, если значение в таблице                                                                                 # обновлялось более last_updated назад, значение не быдет выведено
